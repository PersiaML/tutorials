...
### ID Type Feature
`IDTypeFeature` contains variable length of categorical data. In PERSIA `IDTypeFeature` store the `List[np.array]` data that indeed is a list of list form of sparse matrix. And it only accept the `np.uint64` element. You can add the `user_id`, `photo_id` as follow.
